...
namespace Inventory_API.Controllers
{
    ...
    public class InvoiceController : Controller
    {
        ...
        public InvoiceController(InventoryContext db)
        {
                _db = db;
        }
        ...
        {
            var data = await _db.ViewInvoices.Where(i=>i.CompCode == compcode).ToListAsync();
        ...
        {
            var data = await _db.ViewInvoices.Where(i => i.Id == id).ToListAsync();
        ...
        public async Task<IActionResult> GetSinglebyCode(string code, int uid)
        {
            if (uid > 0)
            {
                ...
                if (compcode != null)
                {
                    var data = await _db.ViewInvoices.Where(i => i.Code == code && i.CompCode == compcode.CompCode).FirstOrDefaultAsync();
                    return Ok(new { data = data });
                }
            }
            return Ok(new { data = "" });
        ...
        {
            var data = await _db.Invoices.Where(i => i.Id == id).FirstOrDefaultAsync();
            if (data != null)
            {
                _db.Invoices.Remove(data);
        ...
        [HttpPost("Update")]
        public async Task<IActionResult> Update([FromBody] Invoice data)
        {

            var model = await _db.Invoices.Where(i => i.Id == data.Id).FirstOrDefaultAsync();
